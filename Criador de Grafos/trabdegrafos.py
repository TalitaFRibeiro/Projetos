...
def preencher_tabela(n, aux, question, n1, n2):
    ...
    if (question == 8):
        ...
        aux = n_cubo(aux, n, 0, 0, n, n,n,log(n,2))
...
def n_cubo(aux, n, i_inicial, j_inicial, i_final, j_final,n_geral,nfora):
    if ((n_geral == 1) or (nfora==1)):
        i_final = i_inicial+1
        j_final = j_inicial+1
        if(nfora==0):
            i_final=i_inicial
        ...
        aux[i_inicial][j_final] = 1
        aux[i_final][j_inicial] = 1
        aux[i_final][j_final] = 0
        
        return aux
    
    else:
        n_g = log(n,2) - 1
        n_cubo(aux,pow(2,n_g),0,0,int(n/2),int(n/2),n_g,nfora)
        n = int(n)
        a = n -1
        iinter = int(n/2)
        jinter = int(n/2)
        
        for i in range(iinter):
            for j in range(iinter):
               
                aux[i+iinter][j+jinter] = aux[i][j]
                aux[j+jinter][i+iinter] = aux[i+iinter][j+jinter]
